...
func (b *Backlog) recover() error {
	...
	for _, entry := range entries {
		...
		if json.Unmarshal(data, &r) != nil || r.Sequence == 0 || r.Submission.Event.EventUUID == "" {
			...
			// diagnosable home for records this backlog cannot use. As in
			// quarantineLocked, a failed move must not be reported as a
			// successful quarantine — record the persistence failure instead
			// of silently losing track of it. loadQuarantineLocked (called
			// right after recover() returns) rescans quarantine/ from disk,
			// so it — not a manual counter here — is what actually counts a
			// successful move.
			dst := filepath.Join(b.cfg.Dir, "quarantine", entry.Name())
			if err := b.renameFile(pendingPath, dst); err != nil {
				b.recordPersistFailureLocked(fmt.Errorf("edgebacklog: quarantine corrupt pending record %s: %w", entry.Name(), platform.WrapDiskError(err)))
			}
			continue
		}
...
// writeFileSynced writes data to name and fsyncs it before returning, unlike
// os.WriteFile which returns as soon as the bytes reach the page cache. It is
// the default for Backlog.writeFile; a test replaces the field directly to
// inject a deterministic ENOSPC without touching this function.
func writeFileSynced(name string, data []byte, perm os.FileMode) error {
