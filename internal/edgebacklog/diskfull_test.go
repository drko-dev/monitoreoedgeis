...
// TestRecoverCorruptPendingQuarantineFailureIsNotSilent is the recovery-time
// counterpart of TestQuarantineMoveFailureDoesNotResurrectTheRecord: a
// corrupt/truncated pending record found by recover() must not be reported
// as successfully quarantined when the move itself fails (e.g. ENOSPC in
// quarantine/'s directory entry). The fault must be installed before Open()
// runs recover(), so this constructs the Backlog by hand instead of using
// openWithFault, which only installs the seam after Open() returns.
func TestRecoverCorruptPendingQuarantineFailureIsNotSilent(t *testing.T) {
	dir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(dir, "pending"), 0o750); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(dir, "quarantine"), 0o750); err != nil {
		t.Fatal(err)
	}
	corruptPath := filepath.Join(dir, "pending", "00000000000000000042.json")
	if err := os.WriteFile(corruptPath, []byte(`{"sequence":42,"submissio`), 0o640); err != nil {
		t.Fatal(err)
	}

	cfg := Config{Dir: dir, MaxOperations: 8, MaxBytes: 1 << 20, RetryBase: time.Millisecond, RetryMax: 2 * time.Millisecond}
	b := &Backlog{cfg: cfg}
	fault := &fsFault{}
	fault.install(b)
	fault.enabled.Store(true)
	...
	if err := b.loadQuarantineLocked(); err != nil {
		t.Fatalf("loadQuarantineLocked: %v", err)
	}
	fault.enabled.Store(false)

	st := b.Status()
	if st.Quarantined != 0 {
		t.Errorf("quarantined = %d, want 0: the move failed, it must not be counted as quarantined", st.Quarantined)
	}
	if st.PersistErrors == 0 {
		t.Error("persist_errors = 0: a failed recovery-time quarantine move must be reported, not swallowed")
	}
	if _, err := os.Stat(corruptPath); err != nil {
		t.Errorf("corrupt record disappeared from pending/ despite the failed move: %v", err)
	}
	if names := quarantineFiles(t, dir); len(names) != 0 {
		t.Errorf("quarantine/ holds %v despite the injected rename failure", names)
	}
}
