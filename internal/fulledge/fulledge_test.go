		...
		Detections: []LocalDetection{
			{
				...
				Tipo:       "persona",
				Confidence: 0.92,
				BBox:       BoundingBox{XMin: 0.1, YMin: 0.2, XMax: 0.5, YMax: 0.8},
			},
		},
	}
...
func TestDeterministicUniqueEventIDs(t *testing.T) {
	...
	res := InferenceResult{
		...
		FrameSeq:     1,
		Detections: []LocalDetection{
			{ClassID: 1, Label: "car", Tipo: "vehiculo", Confidence: 0.85},
			{ClassID: 2, Label: "truck", Tipo: "vehiculo", Confidence: 0.75},
		},
	}
...
func TestBBoxAndConfidencePreserved(t *testing.T) {
	...
	wantBBox := BoundingBox{XMin: 0.12345, YMin: 0.23456, XMax: 0.87654, YMax: 0.98765}
	wantConf := 0.887766
...
func TestEvidencePublicationAndChecksum(t *testing.T) {
	...
	ref, err := evMgr.SaveJPEG("test-event-uuid-1", "cam-east", time.Now().UTC(), rawJPEG)
	if err != nil {
	...
	_, err = evMgr.SaveJPEG("test-event-uuid-1", "cam-east", time.Now().UTC(), rawJPEG)
...
func TestDiskFailureHandling(t *testing.T) {
	...
	res := InferenceResult{
		...
		FrameSeq:     99,
		Detections: []LocalDetection{
			{ClassID: 0, Label: "person", Confidence: 0.95},
		},
	}
