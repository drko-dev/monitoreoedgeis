...
// Config represents the remote configuration payload applied to Edge runtime.
type Config struct {
...
func Validate(cfg Config, knownCameras []string) error {
...
// and validates the resulting Config against known cameras.
func ParseAndValidateJSON(data []byte, knownCameras []string) (Config, error) {
	// First pass: verify no disallowed field is present anywhere
	var rawMap map[string]any
	if err := json.Unmarshal(data, &rawMap); err != nil {
		return Config{}, fmt.Errorf("remoteconfig: json unmarshal error: %w", err)
	}
	...
	if err := dec.Decode(&cfg); err != nil {
		return Config{}, fmt.Errorf("remoteconfig: unknown or invalid field in configuration: %w", err)
	}
