...
func TestTargeted_1_RemoteFPSModifiesSampler(t *testing.T) {
	...
	newFPS := 15.0
	err := adapter.Apply(ctx, RuntimeConfig{
		TargetFPS: &newFPS,
	})
	if err != nil {
...
func TestTargeted_7_InvalidROINoRuntimeModification(t *testing.T) {
	...
	_ = adapter.Apply(ctx, RuntimeConfig{HybridROIs: initialROIs})
...
func TestTargeted_10_PipelineStartFailurePermitsRollback(t *testing.T) {
	...
	// Explicit rollback restores oldFPS
...
func TestTargeted_04_PartialConfigMergeConservesEffectiveConfig(t *testing.T) {
	...
	fps1 := 15.0
	if err := adapter.Apply(ctx, RuntimeConfig{TargetFPS: &fps1}); err != nil {
	...
	w2, h2 := 1280, 720
	if err := adapter.Apply(ctx, RuntimeConfig{OutputWidth: &w2, OutputHeight: &h2}); err != nil {
...
func TestTargeted_05_FailedSecondPartialApplyRestoresPreviousEffectiveConfig(t *testing.T) {
	...
	fps3 := 25.0
	err = failAdapter.Apply(ctx, RuntimeConfig{TargetFPS: &fps3})
	if err == nil {
...
func TestTargeted_NilCloudSinkFailsTransition(t *testing.T) {
	...
	ctx := context.Background()
	cloudMode := config.ModeCloud
	err := adapter.Apply(ctx, RuntimeConfig{ProcessingMode: &cloudMode})
	if err == nil {
...
func TestTargeted_UnreadyVisionSinkFailsTransition(t *testing.T) {
	...
	ctx := context.Background()
	edgeMode := config.ModeEdge
	err := adapter.Apply(ctx, RuntimeConfig{ProcessingMode: &edgeMode})
	if err == nil {
