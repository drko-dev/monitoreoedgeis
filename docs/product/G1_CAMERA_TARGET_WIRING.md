...
| G. Remove (inventory expiry/disappearance → scan success → reconcile → Supervisor stopped) | TESTED | `TestG1B_RemoveByInventoryTTL`, corrected: the precondition (camera live in `rtsp.Manager`) is reached deterministically in setup via explicit `reconciler.reconcile()` calls after each setup scan — never via the credential-sync catch-up-rediscovery path, so setup does not itself depend on `OnScanSuccess`. Only then does the removal phase run: the camera goes silent (not a credential problem), is backdated past `discovery.DeviceTTL` via the new `Inventory.SetLastSeenForTests` test hook, a single scan from a second "silent" Engine sharing the same Inventory finds nothing and prunes it, and — with **no manual `reconcile()` call anywhere near this phase** — `KnownCameras`/the Supervisor going to zero can only have happened through the real `OnScanSuccess → reconcile → SetTargets` path. Also proves a single miss *before* TTL does NOT remove the camera. |
...
| Removal (TTL/disappearance) | `TestG1B_RemoveByInventoryTTL` | Same mutation as the scan-callback guard above (they share the one call site) | **yes — and specifically in the removal phase**: the corrected test's setup (see row G) no longer depends on `OnScanSuccess` at all, so with this mutation applied the precondition is still reached (`started camera stream supervisor`, `connected and playing` both still logged), the silent scan still runs and still prunes the Inventory entry (`pruned=1` still logged) — and only the removal itself never happens, timing out on "camera target to be removed ... via OnScanSuccess alone". The first version of this check failed during setup instead (a real gap the correction fixed: it could not previously tell "removal is broken" apart from "setup never got off the ground"). |
