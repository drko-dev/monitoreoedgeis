...
| **BLOCKED** | **Field/commercial validation only, not a Software 1.0 code blocker.** Hito Z B8 is closed by the explicit no-claim decision: the repository publishes no quantitative Hybrid saving percentage until integrated/field measurement exists. The synthetic 66.00% selector result remains a transport/accounting sensitivity result only. Hito Z B7 is likewise closed by policy: bandwidth controls exist, but no universal production cap is invented; deployments configure one when required. |

### Full Edge — IMPLEMENTED, TESTED, NOT VALIDATED, BLOCKED

| | |
| --- | --- |
| **IMPLEMENTED** | Yes. Local inference is genuinely out of process: the Go agent `exec`s the Python worker and speaks newline-delimited JSON over a single Unix socket. **PyTorch is never inside the Go process** — `go.mod` has zero third-party dependencies, no `import "C"` exists anywhere, and the build is `CGO_ENABLED=0` on every target. The Go agent plus Python Vision Worker split is preserved. As of Hito Z B3, bounded retention exists for the three Full Edge-owned trees (`events/`, `evidence/captures/`, `evidence/clips/`) — count/byte/age bounds, refcounted capture eviction, and edgebacklog-pending-aware eviction, all `0 = disabled` by default; see `docs/product/B3_RETENTION_DESIGN.md`. |
| **TESTED** | Yes for the Go side (worker lifecycle, handshake, restart/backoff, device fields, events, evidence, backlog, sync, quarantine, OTA). As of Hito Z B6, the Python worker's own test suite (health/infer/shutdown protocol, the Unix-socket transport, and the `resolve_device` cpu/cuda/auto contract) runs in CI too, via the `python-vision-worker` job in `.github/workflows/ci.yml` — against `backend.FakeBackend` only, never real Ultralytics/torch/CUDA/a model file (see `deploy/vision-worker/tests/test_worker.py`). Hito Z B3's `RetentionManager` has real-filesystem unit tests (count/byte/TTL bounds, oldest-first, refcounting, pending-record protection, symlink-escape refusal, concurrent create+evict under `-race`, restart/reopen determinism, delete-failure containment) in `internal/fulledge/retention_test.go`. |
...
| **NOT_VALIDATED** | Real model loading; real inference; real CUDA; the packaging path; OTA restart interaction with a live worker; retention under sustained production load / real disk pressure (implemented and locally tested per B3, but never exercised against a real appliance filling a real disk). |
| **BLOCKED** | **The Python runtime is still provisioned by hand, but the packaging half is closed (B2).** `package.sh` now stages the worker's sources (`worker.py`, `backend.py`, `requirements.txt`) into every artifact under `vision-worker/`, `install.sh` places them in the versioned release directory and derives the worker *script* path into the systemd unit, and `scripts/check-vision-runtime.sh` verifies the interpreter and the real dependencies explicitly — failing closed, and never downloading a model. What the package still does **not** and cannot ship is the runtime itself: ultralytics/PyTorch wheels are architecture-specific, so a portable virtualenv across amd64+arm64 cannot exist honestly. No interpreter path is invented, so `GEOCAM_EDGE_YOLO_WORKER_CMD` remains operator-supplied. Weights stay external under `GEOCAM_DATA_DIR/models`. Real inference remains unprovisioned until an operator installs Python and the dependencies on the appliance. |
...
validation, a real pilot and production bandwidth limits all remain open.
Storage retention (G8) was closed by Hito Z B3 — bounded, locally tested,
never validated under real production disk pressure.
...
| **G4** | The remote-config runtime applier does not exist when the video pipeline is disabled. | **CLOSED by Hito Z B4.** The fallback adapter now fails closed on apply, and the existing engine contract reports `rolled_back / APPLY_FAILED` instead of falsely ACKing `applied`. |
...
| **G8** | **CLOSED by Hito Z B3.** `internal/fulledge.RetentionManager` now bounds `events/`, `evidence/captures/` and `evidence/clips/` (count/byte/age, all `0 = disabled` — no commercial figure invented), and the free-disk gate (`GEOCAM_EDGE_MIN_FREE_DISK_BYTES`) now covers clips as well as JPEG captures. See `docs/product/B3_RETENTION_DESIGN.md`. Setting an actual commercial retention period/quota is still deliberately left to an operator (or a later Hito V slice); this closes only the "nothing bounds disk growth" gap. |
