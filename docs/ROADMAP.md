...
- Q4. Imagen preparada. **CODE DONE / REPRODUCIBLE LINUX SPECIFIED** — Pre-packaged release + `install.sh` + systemd + `bootstrap.sh` on standard Linux distributions (Debian 12 minimal / Ubuntu Server / Raspberry Pi OS Lite). No custom ISO needed; fully reproducible image preparation documented in `docs/deployment/appliance.md`.
- Q5. Ethernet + power. **DONE (DOCUMENTED & ARCHITECTED)** — Residential baseline requires wired Ethernet (`eth0`/`enp*`) via DHCP as the primary supported network path (deterministic latency and reliable UDP multicast for WS-Discovery). Generic power requirements specified: 5V/3A USB-C for SBCs, 12V-19V DC barrel for mini-PCs. PoE is explicitly NOT integrated on-board, supported strictly via external Gigabit PoE splitters.
- Q6. Zero-touch enrollment. **CODE DONE / VALIDATED IN TESTS** — Reuses existing `geocam-edge enroll` CLI. On first boot, `geocam-edge-bootstrap.service` runs `bootstrap.sh` before `geocam-edge.service`, checks for ephemeral seed token (`/boot/geocam-enroll.token`, `/boot/firmware/geocam-enroll.token`, `/etc/geocam-edge/enroll.token`, or `GEOCAM_ENROLLMENT_TOKEN`), pipes token to `geocam-edge enroll` (SHA-256 exchanged with SaaS), persists non-root `identity.json` and `credentials.json`, securely shreds/wipes token file, and triggers normal service startup. Covered by targeted tests in `deploy/appliance/bootstrap_test.go`.
