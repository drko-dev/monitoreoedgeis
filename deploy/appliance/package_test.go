...
func TestBuildFfmpegRecipeIsReusedNotDuplicated(t *testing.T) {
	...
	// And no second ffmpeg build recipe may be introduced here. Inspect only
	// non-comment lines: a comment explaining that the LGPL-only recipe is
	// reused NECESSARILY names the flags it avoids, and matching those would
	// fail on correct, well-documented YAML.
	for _, line := range strings.Split(body, "\n") {
		code := strings.TrimSpace(line)
		if strings.HasPrefix(code, "#") {
			continue
		}
		for _, forbidden := range []string{"enable-gpl", "enable-nonfree", "libx264", "libx265"} {
			if strings.Contains(code, forbidden) {
				t.Errorf("release.yml carries its own ffmpeg build flag %q instead of reusing build-ffmpeg-static.sh: %q",
					forbidden, code)
			}
		}
	}
...
// TestVisionRuntimePreflight_SkipImportsIsFailClosed is the regression guard
// for a real defect: the escape hatch used by sandboxed tests printed "readiness
// is NOT established" and then still printed PASS and exited 0, directly
// contradicting the script's own contract ("exit 0 only when every requirement
// is satisfied"). A skipped requirement is not a satisfied one, so the hatch
// must let the run continue to the model-reporting block (which is what makes
// the inert-model test meaningful) while still failing overall.
func TestVisionRuntimePreflight_SkipImportsIsFailClosed(t *testing.T) {
	workerDir := filepath.Join(repoRootForTest(t), "deploy", "vision-worker")
	out, err := runVisionPreflight(t,
		[]string{"GEOCAM_VISION_PREFLIGHT_SKIP_IMPORTS=1"},
		"--worker-dir", workerDir)

	if err == nil {
		t.Fatalf("skip-imports mode exited 0; a skipped requirement must not report readiness:\n%s", out)
	}
	if !strings.Contains(out, "NOT established") {
		t.Errorf("skip-imports mode does not state that readiness was not established:\n%s", out)
	}
	// The success line must be absent, not merely accompanied by a warning.
	if strings.Contains(out, "PASS: interpreter, worker sources and runtime dependencies are all satisfied") {
		t.Errorf("skip-imports mode printed the PASS line, so a false PASS is reachable:\n%s", out)
	}
	// It must still reach the model-reporting block, or the escape hatch would
	// stop being useful to the test that proves no weights are ever fetched.
	if !strings.Contains(out, "model") {
		t.Errorf("skip-imports mode did not reach the model-reporting block:\n%s", out)
	}
}
